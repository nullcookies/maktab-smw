  ...
  <div class="login-box">
    ...
    <div class="login-box-body">
      ...
      <form action="<?php echo $action; ?>" method="post">
        <div class="form-group has-feedback <?php if(!empty($errors['username'])){ ?>has-error<?php } ?>">
          <input class="form-control" placeholder="<?=$this->getTranslation('username')?>" type="text" name="username" value="<?=$username?>" />
          <span class="glyphicon glyphicon-user form-control-feedback"></span>
          <?php if(!empty($errors['username'])){ ?><div class="help-block"><?=$errors['username']?></div><?php } ?>
        </div>
        <div class="form-group has-feedback <?php if(!empty($errors['password'])){ ?>has-error<?php } ?>">
          <input class="form-control" type="password" name="password" placeholder="<?=$this->getTranslation('password')?>"/>
          <span class="glyphicon glyphicon-lock form-control-feedback"></span>
          <?php if(!empty($errors['password'])){ ?><div class="help-block"><?=$errors['password']?></div><?php } ?>
        </div>
        <div class="row">
          <!-- /.col -->
